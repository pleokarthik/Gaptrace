...
from gaptrace_core.coerce import (
    coerce_cache_events,
    coerce_cache_record,
    coerce_chunks,
...
from gaptrace_capture.thread_local import get_active_capture, set_active_capture
...
def set_strict(enabled: bool) -> None:
    ...
    instead of being logged to ~/.gaptrace/errors.log — use it in development
    to surface instrumentation bugs. The default (False) keeps the
    production never-raise contract. The GAPTRACE_CAPTURE_STRICT=1 environment
...
def _strict_enabled() -> bool:
    return _strict or os.environ.get("GAPTRACE_CAPTURE_STRICT", "") == "1"


def _get_logger():
    logger = logging.getLogger("gaptrace-capture")
    if not logger.handlers:
        log_dir = store._gaptrace_dir()
        log_dir.mkdir(parents=True, exist_ok=True)
        handler = logging.FileHandler(str(log_dir / "errors.log"))
        handler.setFormatter(
            logging.Formatter(
                "%(asctime)s [gaptrace-capture] %(message)s",
...
class Capture:
    ...
    Built by gaptrace_capture.start(); each method records one pipeline stage
    onto the underlying RunRecord ("run" stays the data noun). Methods
    never raise unless strict mode is on — failures go to
    ~/.gaptrace/errors.log and the pipeline continues.
    """
    ...
    def chunks(self, chunks: list, requested_count: int | None = None) -> None:
        ...
        filled in (see gaptrace_core.coerce); ChunkRecords pass through
    ...
    def context(
        ...
        gaptrace_core.coerce). Mutates this capture only. Never raises
        unless strict mode is on.
        """
        try:
    ...
    def history(self, pre: list, post: list, eviction_reason: str | None = None) -> None:
        ...
        token counts are estimated unless given (see gaptrace_core.coerce).
    ...
    def commit(self) -> str | None:
        ...
        non-strict mode (the failure is logged to ~/.gaptrace/errors.log);
        ...
        run_seq)`` — see ``gaptrace_core.store.commit_run`` for why this
...
def capture(
    ...
    *,
    ...
    (see gaptrace_core.coerce for the exact coercion rules).
    ...
    failure is logged to ~/.gaptrace/errors.log). Never raises unless strict
