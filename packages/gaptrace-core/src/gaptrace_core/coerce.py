"""Coercion of plain-Python inputs into the gaptrace_core schema dataclasses.

This is the shared user-input boundary: gaptrace_capture's entry points
(Capture methods, capture(), the thread-local proxies) and gaptrace_evaluate's
...
heuristic (no tokenizer dependency — gaptrace-core stays stdlib-only). Pass
explicit ``tokens`` / ``token_count`` values to override.

All functions are pure and raise TypeError/KeyError on unusable input;
callers decide the failure policy (gaptrace_capture swallows/logs by default
and raises in strict mode; gaptrace_evaluate raises ValueError).
"""
