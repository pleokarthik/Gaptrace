...
from gaptrace.cli import main
from gaptrace_core import store as core_store
from gaptrace_core.schema import CacheRecord, RunRecord
...
class TestFind:
    ...
    def test_date_filters(self, gaptrace_home):
...
class TestExplain:
    ...
    def test_html_output(self, populated_db, gaptrace_home):
        result = CliRunner().invoke(main, ["explain", "s2r1", "--html"])
        assert result.exit_code == 0
        assert "Report written to" in result.output
        reports_dir = gaptrace_home / ".gaptrace" / "reports"
...
class TestExplainHtmlMinimalRecord:
    def test_explain_html_minimal_record(self, gaptrace_home):
...
class TestExplainEvalScores:
    def test_explain_shows_eval_scores_when_present(self, gaptrace_home):
...
class TestExplainSemanticCache:
    ...
    def test_no_cache_data_skips_panel_silently(self, gaptrace_home):
    ...
    def test_checked_miss_renders_panel(self, gaptrace_home):
    ...
    def test_checked_hit_borderline_explains_why(self, gaptrace_home):
        self._insert(
            CacheRecord(
                checked=True,
                hit=True,
    ...
    def test_checked_hit_stale_explains_why(self, gaptrace_home):
