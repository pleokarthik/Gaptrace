"""Task-level public API of ragradar-evaluate.
...
from ragradar_core.coerce import coerce_run_record
...
from ragradar_evaluate.policy.risk import compute_risk_score
...
class EvalResult:
    ...
    def to_eval_scores(self) -> dict:
        ...
        (and read back by `ragradar explain`). Pure."""
        input_values: dict = {}
...
def _resolve_target(target) -> tuple[RunRecord, str | None, tuple | None, str]:
    ...
    object with a .run_id attribute (e.g. a ragradar_capture.Capture).
    Hand-built RunRecords are normalized through ragradar_core.coerce, so
    primitive chunks/turns (the same shapes ragradar_capture.capture()
...
def evaluate(
    ...
    *,
    ...
    ragradar_capture.Capture. metrics=None runs every metric applicable to
    ...
    via ragradar_core.store.write_eval_scores — the only persistence path.
    ...
    if save and run_key is None:
        raise ValueError(
            ...
            "id (e.g. the string ragradar_capture.capture() returned)."
        )
...
def _learned_thresholds(pipeline: str) -> dict:
    ...
    if len(store.get_all_evaluated_runs(pipeline)) >= 10:
        try:
            from ragradar_evaluate.benchmark import builder
