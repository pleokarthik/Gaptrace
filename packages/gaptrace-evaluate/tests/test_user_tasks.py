...
public API (gaptrace_capture.capture / gaptrace_evaluate.check / evaluate /
available_metrics). No raw SQL, no internal modules."""

import gaptrace_capture
from gaptrace_evaluate import check, evaluate
from gaptrace_evaluate.facade import INPUT_METRICS
...
class TestChunklessCheckDoesNotCrash:
    def test_verdict_explains_insufficient_data(self):
        run_id = gaptrace_capture.capture("bare question", "bare answer")
        result = check(run_id)
