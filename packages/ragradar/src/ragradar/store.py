"""Read-side store queries for the ragradar analyst CLI.

Connection handling, schema, and the shared run getters live in
ragradar_core.store — this module only adds the CLI-specific queries
(session listing, search, target resolution, session rename).
"""

from ragradar_core.store import (  # noqa: F401 (get_run/get_latest_run re-exported for callers)
    connect,
    get_latest_run,
    get_run,
)
...
def search_runs(
    ...
    Read-only query (though connecting may create/migrate the store).
    FTS5 is always available: ragradar_core guarantees the schema is at the
    latest version, which ships the runs_fts index.
    """
    from ragradar.find.query_builder import build_search_query
...
def resolve_target(target: str | None = None) -> dict | list[dict] | None:
    ...
    if len(results) > 1:
        from ragradar.find.bm25 import score
