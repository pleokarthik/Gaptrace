...
from gaptrace_capture.thread_local import clear_active_capture, get_active_capture
from gaptrace_core import coerce, store
from gaptrace_core.schema import RunRecord, TokenBudget
...
def non_strict():
    ...
    gaptrace_capture.set_strict(False)
    yield
    gaptrace_capture.set_strict(False)
...
class TestCapture:
    def test_minimal_capture(self):
        gaptrace_capture.capture("what is RRF?", "reciprocal rank fusion")
    ...
    def test_returns_valid_run_id_matching_db(self):
        run_id = gaptrace_capture.capture("q", "r", pipeline="pipe")
    ...
    def test_unknown_kwarg_raises_type_error(self):
        with pytest.raises(TypeError):
            gaptrace_capture.capture("q", "r", chunk=[{"chunk_id": "c1"}])
...
class TestStartAndCapture:
    ...
    def test_staged_flow_run_id_lifecycle(self):
        cap = gaptrace_capture.start("test query", pipeline="test")
    ...
    def test_double_commit_same_id_writes_once(self):
        cap = gaptrace_capture.start("q", pipeline="test")
...
class TestPrimitiveInputs:
    ...
    def test_capture_one_liner_requested_chunk_count_ignored_without_chunks(self):
        run_id = gaptrace_capture.capture("q", "r", requested_chunk_count=5)
        assert run_id is not None
        data = self._stored_record()
    ...
    def test_chunk_without_content_still_errors(self):
        gaptrace_capture.set_strict(True)
        cap = gaptrace_capture.start("q", pipeline="test")
        with pytest.raises(TypeError):
    ...
    def test_cache_mapping_and_tuples(self):
        ...
        cap = gaptrace_capture.start("q2", pipeline="test")
        cap.cache([("c3", 1)])
        cap.response("r2")
        events = self._stored_record()["cache_events"]
...
class TestFailureSilence:
    def test_capture_failure_never_raises_and_returns_none(self, monkeypatch):
        ...
        monkeypatch.setattr("gaptrace_core.store.commit_run", fail)
        assert gaptrace_capture.capture("q", "r") is None
...
class TestStrictMode:
    ...
    def test_env_var_enables_strict(self, monkeypatch):
        monkeypatch.setenv("GAPTRACE_CAPTURE_STRICT", "1")
        cap = gaptrace_capture.start("q", pipeline="test")
        with pytest.raises(TypeError):
            cap.chunks([{"bad_field": 1}])

    def test_non_strict_logs_to_errors_log(self, gaptrace_home):
        cap = gaptrace_capture.start("q", pipeline="test")
        cap.chunks([{"bad_field": 1}])  # swallowed

        errors_log = gaptrace_home / ".gaptrace" / "errors.log"
...
class TestThreadLocal:
    ...
    def test_proxy_routes_to_active_capture(self):
        cap = gaptrace_capture.start("proxy test", pipeline="test")
        gaptrace_capture.chunks(
            [
                {
                    "chunk_id": "c1",
                    "source_doc_id": "d1",
    ...
    def test_metadata_filter_proxy_routes_to_active_capture(self):
        cap = gaptrace_capture.start("proxy test", pipeline="test")
        gaptrace_capture.metadata_filter(applied=True, candidate_count=10, excluded_count=3)
    ...
    def test_proxy_without_active_capture_is_silent(self):
        gaptrace_capture.chunks([])
        gaptrace_capture.context("prompt")
        gaptrace_capture.tool_call({"tool_name": "search", "arguments": {}})
        gaptrace_capture.semantic_cache(checked=True)
        assert gaptrace_capture.response("r") is None
        assert gaptrace_capture.commit() is None

    def test_proxy_without_active_capture_logs(self, gaptrace_home):
        gaptrace_capture.chunks([])
        errors_log = gaptrace_home / ".gaptrace" / "errors.log"
        assert errors_log.exists()
    ...
    def test_context_proxy_budget_flow(self):
        cap = gaptrace_capture.start("proxy q", pipeline="test")
        gaptrace_capture.context(
            "final prompt",
            {
                "total_limit": 100,
                "chunks_allocated": 50,
        ...
        run_id = gaptrace_capture.response("resp")
        assert run_id is not None
